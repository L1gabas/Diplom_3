package site.nomoreparties.stellarburgers.commons;

public class EpAndApi {

    public static final String TEST_STAND = "https://stellarburgers.nomoreparties.site/";
    public static final String REFRESH_URL = "https://stellarburgers.nomoreparties.site/login";
    public static final String USER_CREATE = "/api/auth/register";
    public static final String USER_DATA = "/api/auth/user";
    public static final String USER_LOGIN = "/api/auth/login";
}
